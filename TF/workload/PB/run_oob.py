            ...
            for dtype in datatypes:
                for model_name in model_list:
                    if target_model == "all" or target_model == model_name:
                        bench_cmd = "timeout 15m ./launch_benchmark.sh --framework=tensorflow --model_name={model_name} --mode_name=throughput --precision={data_type} --batch_size={batch_size} --numa_nodes_use=0 --cores_per_instance={core_per_instance} --num_warmup=0 --num_iter=1 --channels_last=1 --profile=0 --dnnl_verbose=0".format(
                            batch_size=bs,
